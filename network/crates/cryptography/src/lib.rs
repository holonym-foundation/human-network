...
pub fn hash256<T: AsRef<[u8]>>(preimage: T) -> [u8; 32] {
    let mut hasher = Sha3_256::new();
    hasher.update(preimage);
    hasher.finalize().as_slice().try_into().unwrap() //should never panic
}
/// Local KDF based on username (or identifier / salt preimage) and password (or secret) using Argon2 for key derivation.
pub fn local_kdf<T: AsRef<[u8]>>(username: T, password: T) -> Result<String, Error> {
    ...
    let salt_string = argon2::password_hash::SaltString::encode_b64(&salt).map_err(|_| Error::InvalidInput)?;
    let derived = argon2.hash_password(password.as_ref(), &salt_string).map_err(|_| Error::InvalidInput)?;
