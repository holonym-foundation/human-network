...
impl JWTClaims {
    ...
    pub fn from_raw_token_unchecked(token: &str) -> Result<Self, anyhow::Error> {
        ...
        Ok(Self {
            ...
            from_enclave: claims.get("from_enclave").map(|v| v.as_bool().unwrap_or(false)),
        })
    }
    ...
    pub fn unique_string(&self) -> Result<String, anyhow::Error> {
        ...
        Ok([login_at_domain, self.iss.clone(), self.sub.clone(), self.salt.clone().unwrap_or("human-default-salt".to_string())].join(","))
    }
}
#[cfg(test)]
mod tests {
