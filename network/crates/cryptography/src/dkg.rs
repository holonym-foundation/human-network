use crate::serialization::point_hex;
use crate::{
...
impl Network {
    pub fn new(t: usize, n: usize, is_tstar: bool) -> Result<Self, Error> {
        let set = HashMap::new();

        info!("Initializing: t={}, n={}", t, n);

        let invalid = if is_tstar { t > n || n < 2 || t == 0 } else { t >= n || n <= 2 || t == 0 };

        if invalid {
            info!("Validation failed: is_tstar={}, t={}, n={}", is_tstar, t, n);
            return Err(Error::Other(format!("Invalid parameters for {} network. t={}, n={}", if is_tstar { "tstar" } else { "new" }, t, n)));
        }

        info!("Validation passed: threshold={}, total_nodes={}", t, n);
...
impl<const N: usize, C: Curve<N>> Node<N, C> {
    ...
    pub fn with_rsa_key_and_keyshare(id: u128, rsa_seckey: RsaPrivateKey, keyshare: Vec<u8>, network_pubkey: Vec<u8>) -> Result<Self, DKGError> {
        let rsa_pubkey = RsaPublicKey::from(&rsa_seckey);
        let keyshare_data = C::FieldEl::from(C::Scalar::from_bytes(&keyshare).unwrap());
        // let keyshare_data = C::FieldEl::from(C::Scalar::from_biguint_vartime(BigUint::from_bytes_be(&keyshare)).unwrap());
        info!("Keyshare data restored :{:?}", keyshare_data);
        let network_pubkey = C::Point::from_encoded(&network_pubkey).map_err(|e| DKGError::Other(format!("Failed to restore network_pub key {}", e)))?;
        Ok(Self {
            id,
            rsa_pubkey,
            rsa_seckey,
            init_pub_share: C::ZERO,
            private_share: keyshare_data,
            network_pubkey,
            own_partial_share: C::FieldEl::random(&mut thread_rng()),
        })
    }
...
mod test {
    ...
    fn test_full_2_3_dkg() {
        ...
        let mut threshold = Network::new(2, 3, false).unwrap();
    ...
    fn test_full_5_3_dkg() {
        ...
        let mut n2: Node<32, Secp256k1> = Node::new(2).unwrap();
        let mut n3: Node<32, Secp256k1> = Node::new(3).unwrap();
        let mut threshold = Network::new(5, 3, false).unwrap();
        threshold.add_node(&n1);
        threshold.add_node(&n2);
        threshold.add_node(&n3);
        let (y1, v1, y11) = &n1.dkg_round_1(&threshold).unwrap();
    ...
    fn test_full_3_10_dkg() {
        ...
        let mut threshold = Network::new(3, 10, false).unwrap();
        threshold.add_node(&n1);
        threshold.add_node(&n2);
        threshold.add_node(&n3);
